...
template <typename T>
using type_map = typename std::conditional<
  std::is_same_v<EXWebGLClass, T>,
  UEXGLObjectId,
  T
>::type;
...
template <typename T>
inline std::
    enable_if_t<!(is_integral_v<T> || std::is_floating_point_v<T> || is_supported_vector<T>), type_map<T>>
...
template <>
inline UEXGLObjectId unpackArg<EXWebGLClass>(
    jsi::Runtime &runtime,
    const jsi::Value *jsArgv) {
  if (!jsArgv->isObject() || !jsArgv->asObject(runtime).hasProperty(runtime, "id")) {
    throw std::runtime_error("value is not an WebGLObject");
  }
  return static_cast<UEXGLObjectId>(jsArgv->asObject(runtime).getProperty(runtime, "id").asNumber());
}
